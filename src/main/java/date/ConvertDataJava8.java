...
public class ConvertDataJava8 {
	...
	private static String updateYearJava8(String stringDate) throws Exception {
		String yearDate = Optional.ofNullable(stringDate).filter(value -> stringDate.split("/").length == 3).map(yearValue -> yearValue.split("/")[2]).orElseThrow(() -> new Exception("Erro dd/MM/yy or dd/MM/yyyy"));
		return LocalDate.parse(getNewDate(stringDate, getYear(yearDate)), DateTimeFormatter.ofPattern(PATTERN_DDMMYYYY)).format(DateTimeFormatter.ofPattern(PATTERN_YYYYMMDD));
	}
	...
	private static String getYear(String yearDate) {
		return Optional.ofNullable(yearDate).filter(yearValue -> yearValue.length() <= 2).map(ConvertDataJava8::convertYear).orElse(yearDate);
	}
	
	private static String convertYear(String yearToConvert) {
		return Optional.of(yearToConvert).filter(yearInt -> (LocalDate.now().getYear() - Integer.valueOf(yearInt)) < 2000).map(newYear -> "19" + newYear).orElseGet(() -> "20".concat(yearToConvert));
	}
	
	
	private static String convertStrDate(String date, String formatIn, String formatOut) {
        return LocalDate.parse(date, DateTimeFormatter.ofPattern(formatIn)).format(DateTimeFormatter.ofPattern(formatOut));
    }
	...
	private static String getDefaultFormatTime(String date) {
		return ZonedDateTime.parse(date, DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss")).with(LocalTime.of(0, 0, 0, 0)).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
	}
	...
	private static String getDefaultDBFormatTime4(String date) throws ParseException {
		try {
			...
			return newDate.toInstant().atZone(ZoneId.systemDefault()).format(DateTimeFormatter.ofPattern(PATTERN_DDMMYYYY));
	...
	private static void checkDates() {
		...
		String testFutureDate = LocalDate.parse("10/10/2100", DateTimeFormatter.ofPattern(PATTERN_DDMMYYYY)).plusDays(1).format(DateTimeFormatter.ofPattern(PATTERN_DDMMYYYY));
	...
	public static boolean isValid(String dateValue) {
			...
			boolean isBeforeDate = isValidFormat && Optional.ofNullable(dateValue).map(dateToConvert -> LocalDate.parse(dateToConvert, DateTimeFormatter.ofPattern(PATTERN_DDMMYYYY)))
						.filter(localDate -> beforeOrEqualThanToday && localDate.isAfter(LocalDate.now())) .map(result -> false)
						.orElse(true);
