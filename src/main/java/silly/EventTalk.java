...
public class EventTalk {
	...
	private List<Talk> afternonEvents;
	...
	private void generateAfternoon() {
		this.afternonEvents = new LinkedList<>();
		final var totalDuration = new AtomicInteger(0);
		...
		while (totalDuration.intValue() < MAX_AFTERNOON_MINUTES || totalDuration.intValue() > MAX_AFTERNOON_MINUTES) {
			Optional.of(totalDuration)
				...
				.ifPresentOrElse(duration -> addRandomTalk(afternonEvents, duration)
						, () -> removeLastTalk(afternonEvents, totalDuration));
		}
	}
	...
	private void generateDayEvent() {
		...
		Optional.of(lunch).map(this::convertTime).ifPresent(dayEvent::add);
		afternonEvents.stream().map(this::convertTime).forEach(dayEvent::add);
		Optional.of(networkingEvent).map(this::convertTime).ifPresent(dayEvent::add);
	}
